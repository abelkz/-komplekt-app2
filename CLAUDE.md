...
>
> Это **единственное**, что осталось между рабочими пушами и доставкой
> уведомлений о снижении цены: устройство регистрируется, токен в базе
> есть, триггер пишет `price_drops` — некому только отправить.
...
- `send-broadcast` — объявление всем, у кого установлено приложение;
  зовётся из вкладки «Рассылка» в админке, секреты те же `FCM_*`.
  **Права проверяет сама функция**: она ходит в базу с `service_role`,
  то есть мимо RLS, и без проверки роли разослать пуш всем смог бы любой,
  кто знает её адрес. Спрятанная кнопка защитой не является;
- `photo-search` — поиск по фото (в разработке).

### Как работают пуши о снижении цены

> **Регистрация устройства работает — проверено 20.09.2026 на сборке 37.**
> В `device_tokens` появилась первая строка за всё время, тестовый пуш из
> консоли Firebase дошёл до iPhone. Отправки о снижении цены пока нет:
> для неё нужен деплой `send-price-alerts` и Database Webhook, см. ниже
> и `ПУШИ_И_ФУНКЦИИ.md`.
>
> **Чего это стоило — чтобы не искать заново.** Пуши не работали с первого
> релиза, и причин оказалось три, одна под другой:
>
> 1. Не было `lib/firebase_options.dart`, а нативные конфиги положить некуда
>    (`android/`/`ios/` генерируются каждой сборкой, §6.4). Файл написан
>    руками, значения — из консоли Firebase.
> 2. На iOS не хватало `aps-environment` в entitlements и возможности Push
>    у App ID. Добавлено под флагом `PUSH_READY` в `codemagic.yaml`.
> 3. **Главное и самое неочевидное.** `flutter create` теперь генерирует
>    iOS-приложение на **UIScene**: плагины регистрируются не при запуске,
>    а позже, в `didInitializeImplicitFlutterEngine`. А `firebase_messaging`
>    до 16.7.0 запускал `registerForRemoteNotifications` из обработчика
>    `UIApplicationDidFinishLaunchingNotification` — к моменту регистрации
>    плагина это уведомление уже прошло, обработчик не срабатывал, и
>    регистрация в APNs **не запускалась никогда**. Молча: Apple не
>    отказывает, она просто ничего не спрашивает, `getAPNSToken()`
>    возвращает `null` без исключения, а `getToken()` падает с
>    `apns-token-not-set`. Закрыто подъёмом до `firebase_messaging` 16.7.0
>    («register for APNs when UIScene plugins miss launch callbacks»),
>    он же потянул `firebase_core` 4.x и минимальную iOS 15.0.
>
> Мораль: если APNs-токена нет, а разрешение выдано и пуши от других
> приложений приходят — смотреть не на ключи и профили, а на то,
> **вызывается ли регистрация вообще**. Права подписи печатает в лог шаг
> «Права подписанного приложения в лог» в `codemagic.yaml`.
...
Уведомления идут **только по избранному** — подборки триггер не смотрит.
И только на снижение: повышение цены ничего не запускает.

> **Триггер в живой базе был не тот, что в репозитории — закрыто `0038`.**
> В `0006` лежит `on_offer_price_drop`, читающая несуществующую
> `public.users`; в живой базе работала переписанная `offers_notify_price_drop`
> (через `profiles`). Текст снят с базы 19.09.2026 и зафиксирован миграцией.
> Заодно исправлено: строки вставлялись со `status = 'sent'` — то есть
> помеченными отправленными до отправки, из-за чего запасной путь в
> `send-price-alerts` (он ищет `pending`) не находил ничего. Теперь
> `pending`, а `sent` проставляет сама функция после доставки.
...
1. **Схема живой базы расходится с миграциями.** Живую базу использует ещё и
   ...
   Навести порядок (переименовать, склеить дубликаты, удалить пустую)
   можно во вкладке «Марки» админки — миграция `0035`.

   **Сюда же — про счётчики идентификаторов.** У `brands` последовательность
   стояла на 1 при `max(id) = 6`: шесть марок приехали из начального
   наполнения с явными id, а через `nextval` в эту таблицу никто никогда не
   вставлял. Пока приложение марки только читало, это было незаметно; первая
   же попытка завести новую из формы товара падала с
   `23505: duplicate key ... brands_pkey`. Починено миграцией `0036`.
   У `products`, `offers` и `suppliers` счётчики проверены тогда же и в
   порядке — эти таблицы приложение наполняет само. Мораль: если таблицу
   когда-то наполняли SQL-ом с явными id, перед первой вставкой из кода
   сверить `max(id)` с `last_value` из `pg_sequences`.
...
12. **Админка `/admin` работает, SQL-ом одобрять не нужно.** Семь вкладок:
    «Сводка», «Поставщики» (одобрить / отклонить / вернуть на проверку),
    «Тарифы», «Буст», «Жалобы», «Марки» и «Рассылка». Ожидающие решения
    сортируются наверх, их число видно счётчиком в профиле
    (`adminPendingCountProvider`).

    «Рассылка» стоит последней нарочно: это единственное действие в панели,
    которое уходит наружу к живым людям и **не отменяется**. Перед отправкой
    показывается число получателей и предпросмотр — опечатку в уже
    отправленном пуше исправить нельзя. Журнал отправок — таблица
    `broadcasts` (миграция `0039`).
...
- **Вкладка «Марки» в админке** (`0035`) — переименовать, склеить
  дубликаты, удалить пустую; завести марку заранее, без товара (`0037`) —
  чтобы положить в подсказки правильное написание до того, как поставщик
  изобретёт своё. Плюс `0036` — починка счётчика id у `brands`,
  без которой новые марки вообще не заводились (см. §6.1).
- **Зафиксированы версии пакетов** — `pubspec.lock` в git (см. §6.5).
- **Пуши на iOS заработали** (20.09.2026) — разбор в §5. Устройство
  регистрируется, токен доходит до `device_tokens`, тестовый пуш получен.
- **Рассылка объявлений** (`0039`) — вкладка «Рассылка» в админке и
  Edge Function `send-broadcast`: заголовок, текст, сегмент (все /
  клиенты / поставщики), журнал отправок. **Заработает только после
  деплоя функции** и секретов `FCM_*`; до этого кнопка честно скажет,
  что рассылать нечем. И до выката новой версии в стор аудитория почти
  пустая: пуши принимают лишь сборки начиная с 37-й.
