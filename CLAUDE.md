...
Приложение доведено до подачи в App Store: последние коммиты — закрытие
замечаний ревью (1.2 модерация пользовательского контента, 3.1.1 покупки,
ITMS-90683 описания геопозиции), вход через Apple, сборка только под iPhone,
иконки и подпись в Codemagic. Веб-версия и Android-APK собираются
автоматически.

**`ROADMAP.md` частично устарел** — он написан, когда код ещё ни разу не
компилировался, и многое из его P0/P1 давно закрыто (CI, RLS-триггеры,
удаление аккаунта, полнотекстовый поиск, Realtime, настройки уведомлений,
скелетоны). Читать его как список идей, а не как актуальный статус.

Что действительно осталось крупного:
