...
12. **Одобрение поставщика — вручную.** Есть экран `/admin` и админские RPC, но
    базовый путь — SQL:
...
2. **Жалобы на отзывы.** Таблица `content_reports` заведена миграцией `0025`
   ...
   передаёт `ReportMenu`. Разбор — сменой статуса администратором;
   **экрана для разбора пока нет**, жалобы видны только запросом к таблице.
...
4. **Организация Supabase на тарифе FREE.** Проект уже дважды вставал: один раз
...
Миграция `0024` **уже применена** к живой базе (18.09.2026, через SQL Editor) —
таблица `app_events` создана. **Миграция `0025` (жалобы) ещё НЕ применена** —
без неё жалобы по-прежнему никуда не доходят. Но записи в неё пойдут только после выката 1.0.1:
код, который пишет события, есть лишь в новой версии, а в сторе пока 1.0.0.
